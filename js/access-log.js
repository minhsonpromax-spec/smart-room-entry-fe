document.addEventListener("DOMContentLoaded", () => {
  ...
  const unlockMethodSelect = document.getElementById("unlockMethod");
  ...
  const fetchData = async (page = 1, size = pageSize) => {
    ...
    const endDate = endDateSelect.value
      ...
      : "";
    const unlockMethod = unlockMethodSelect?.value || "";
    ...
    const params = new URLSearchParams({
      ...(startDate && { startDate }),
      ...(endDate && { endDate }),
      ...(unlockMethod && { unlockType: unlockMethod }),
      ...(search && { search }),
      page: String(page),
    ...
    try {
      const res = await fetch(`${baseUrl}/access-logs?${params.toString()}`, {
        headers: {
  ...
  <div class="text-sm text-gray-700 mb-1 flex items-center gap-1">
    ...
    <span><b>Thẻ:</b> ${
      log.userName || "Không rõ"
    } - Phương thức: ${translateUnlockMethod(log.unlockMethod)}</span>
  </div>

  <div class="text-sm text-gray-700 mb-1">
    <b>Vào:</b> ${log.inCount} - <b>Ra:</b> ${log.outCount}
  </div>
...
`;
        ...
        <td class="px-4 py-3 text-center">${log.inCount}</td>
        <td class="px-4 py-3 text-center">${log.outCount}</td>
        <td class="px-4 py-3 font-semibold text-center">${translateUnlockMethod(
          log.unlockMethod
        )}</td>
  ...
  unlockMethodSelect?.addEventListener("change", debouncedFetch);
  ...
  const translateUnlockMethod = (method) => {
    switch (method) {
      case "FINGERPRINT":
        return "Vân tay";
      case "CARD":
